...
#     pack: int | None = Field(None, description="Pack size, e.g. 6, 12", examples=[12])
...
#     pack: int | None = Field(None, description="Pack size, e.g. 6, 12", examples=[12])
#     unit_in_case: int | None = Field(None, description="Number of units per case configuration", examples=[12])
#     case_qty: int | None = Field(None, description="Quantity of full cases ordered/shipped", examples=[2])
#     unit_qty: int | None = Field(None, description="Quantity of individual split units ordered/shipped", examples=[5])
#     unit_cost: float | None = Field(None, description="Unit price per item/case", examples=[25.50])
#     discount: float | None = Field(None, description="Discount amount or percentage applied", examples=[5.00])
#     tax: float | None = Field(None, description="Tax amount applied to this line", examples=[1.25])
#     net_unit_cost: float | None = Field(None, description="Net cost per bottle after adjustments", examples=[24.00])
...
def _to_int(v: Any) -> int | None:
    """Coerce LLM output to int. Handles int, float, str, None."""
    if v is None:
        return None
    try:
...
def _to_float(v: Any) -> float | None:
    """Coerce LLM output to float. Handles float, int, str, None."""
    if v is None:
        return None
    try:
...
def _to_str(v: Any) -> str | None:
    """Coerce LLM output to str. Handles int, float, str, None."""
    if v is None:
        return None
...
class InvoiceLineItem(BaseModel):
    ...
    # --- int coercions ---
    @field_validator("line_no", "pack", "unit_in_case", "case_qty", "unit_qty", mode="before")
    @classmethod
    def coerce_int(cls, v: Any) -> int | None:
        return _to_int(v)

    # --- float coercions ---
    @field_validator("unit_cost", "discount", "tax", "net_unit_cost", "total_cost", "rip", mode="before")
    @classmethod
    def coerce_float(cls, v: Any) -> float | None:
        return _to_float(v)

    # --- str coercions (guards against LLM returning int for UPC/code fields) ---
    @field_validator("item_distributor_code", "upc", "item_name", "size", mode="before")
    @classmethod
...
class InvoiceSummary(BaseModel):
    ...
    # --- int coercions ---
    @field_validator("total_items", "total_out_of_stocks", mode="before")
    @classmethod
    def coerce_int(cls, v: Any) -> int | None:
        return _to_int(v)
    ...
    # --- float coercions ---
    @field_validator(
        "total_case", "total_units", "total_rip", "sub_total",
        "total_discounts", "total_tax", "deposit", "total_payable",
        mode="before"
    )
    @classmethod
    def coerce_float(cls, v: Any) -> float | None:
...
class UniversalInvoice(BaseModel):
    ...
    # --- str coercions (route_no can come back as int from LLM) ---
    @field_validator("invoice_no", "route_no", "invoice_date", "due_date", "distributor_name", mode="before")
    @classmethod
    def coerce_str(cls, v: Any) -> str | None:
        return _to_str(v)